...
* Single patient prediction from manual feature inputs
...
* `Single Prediction`:
  ...
  * feature input form near the top (age, gender, BMI, HbA1c, blood glucose, hypertension, heart disease, smoking history)
...
The app must allow the user to manually enter the model features for one patient and generate:
...
The app must allow the user to upload a `.csv` file containing multiple patients and the required model features.

The app must return a downloadable CSV with one row per patient and at least:
...
* include a small set of example patients with the required inference columns
