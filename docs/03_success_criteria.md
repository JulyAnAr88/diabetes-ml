...
* Includes diabetes-focused analysis
...
* Reports `precision`, `recall`, and `F1` for the diabetes class
* Final model improves on the baseline on validation and keeps that improvement on test
* Target test `ROC-AUC >= 0.80`
* Target diabetes-class `recall >= 0.60` at the selected decision threshold
